...
\midrule
1 & Transformer architecture（Transformer 架构） & 需要能说明它指向的对象、参与的公式或代码位置，以及一个典型失败情形。\\
2 & pre-norm（前归一化） & 需要能说明它指向的对象、参与的公式或代码位置，以及一个典型失败情形。\\
3 & RMSNorm & 需要能说明它指向的对象、参与的公式或代码位置，以及一个典型失败情形。\\
4 & RoPE（rotary positional embedding） & 需要能说明它指向的对象、参与的公式或代码位置，以及一个典型失败情形。\\
5 & SwiGLU & 需要能说明它指向的对象、参与的公式或代码位置，以及一个典型失败情形。\\
6 & hyperparameter（超参数） & 需要能说明它指向的对象、参与的公式或代码位置，以及一个典型失败情形。\\
\bottomrule
\end{longtable}
...
现在进入 Decoder-only Transformer 的骨架。本节关心的不是名称本身，而是它把 现代 Transformer architecture 与 hyperparameters 中的哪些对象变成可计算、可测量或可优化的量。
...
视频把 architecture 讲成现代论文中大量细节的集合，而不是单一的 Transformer block 图。 这给出本节的对象和边界。读者需要把其中的名词对应到张量、数据记录、训练状态或系统状态，而不是停留在缩写层面。
decoder-only LM 的主干通常是 token embedding、若干层 causal self-attention 和 MLP、norm、residual connection、LM head。 因而同一个方法在小规模示例、单机实验和集群训练中的意义并不相同。比较方法时，问题规模、硬件条件、数据分布和评估口径必须同时给出。
课程材料强调许多看似小的选择会影响 scale：normalization placement、activation、position encoding、head layout、optimizer schedule。 这使本节具有可检验性：如果一个结论不能在公式、伪代码、profile trace、benchmark 或 ablation 中表现出来，它就还只是直觉。
因此，Decoder-only Transformer 的骨架 应当被理解为一组可以实现和测试的假设。CS336 反复强调 from scratch，正是因为只有能被复现的机制，才可能被稳定改进。
...
检验 Decoder-only Transformer 的骨架 的第一步是确定测量对象。这里的测量对象通常不是一个抽象名词，而是与 architecture、Transformer、attention、MLP、residual 有关的张量、样本、请求、奖励、通信操作或评估记录。如果对象没有被具体化，后面的实验就只能得到描述性观察，不能得到可复现结论。
第二步是构造最小输入。最小输入应该足够小，使读者能手算或打印关键中间量；同时又要保留本节机制。对于 Decoder-only Transformer 的骨架，最小输入不追求逼真规模，而追求暴露因果关系：改变一个变量时，哪些输出、成本或错误会随之改变。
第三步是写出资源账本。账本可以是 FLOPs、bytes moved、显存峰值、通信量、token 数、样本数、reward 分布或 benchmark 分数。符号说明：\textbackslash{}(Q,K,V\textbackslash{}) 是 query/key/value；\textbackslash{}(d\_k\textbackslash{}) 是 head dimension；\textbackslash{}(M\textbackslash{}) 是 causal mask，用于禁止看见未来 token。 这类说明应被转化为单位明确的数量关系；如果数量只能间接观测，就必须说明使用了什么 proxy。
...
第五步是观察失败条件。教材中的成功路径往往最容易记住，但工程中真正决定方法边界的是失败案例。本节至少要记住这些限制：公开 slides 总结的是主流设计，不代表所有 frontier models 的真实 recipe；architecture comparison 必须控制参数量、训练 tokens、数据和 optimizer，否则容易把 scaling 效应误判为结构效应。复习时可以把这些限制改写成反例测试。
最后，把实验结论放回 现代 Transformer architecture 与 hyperparameters 的整体结构中。Decoder-only Transformer 的骨架 不是孤立技巧：它会影响训练目标、数据选择、系统成本或评估解释中的至少一项。能说清楚这种影响，才说明读者已经从名词记忆进入机制理解。
\subsection{常见混淆：Decoder-only Transformer 的骨架}
architecture 在本节中应被看作一个技术对象，而不是一个孤立名词。需要判断它指向模型、数据、系统还是评估，并说明它在“Decoder-only Transformer 的骨架”中改变了哪个变量或约束。
Transformer 是现代 LM 的基本计算骨架。在“Decoder-only Transformer 的骨架”中，这个概念用于解释 现代 Transformer architecture 与 hyperparameters 的一个具体机制。
attention（注意力）提供 token 间交互，但长上下文下成本很高。在“Decoder-only Transformer 的骨架”中，这个概念用于解释 现代 Transformer architecture 与 hyperparameters 的一个具体机制。
MLP 在本节中应被看作一个技术对象，而不是一个孤立名词。需要判断它指向模型、数据、系统还是评估，并说明它在“Decoder-only Transformer 的骨架”中改变了哪个变量或约束。
residual 在本节中应被看作一个技术对象，而不是一个孤立名词。需要判断它指向模型、数据、系统还是评估，并说明它在“Decoder-only Transformer 的骨架”中改变了哪个变量或约束。
这些概念的共同点是：只有进入公式、代码或评估协议之后才有明确含义。单独背诵名称不会帮助判断一个训练 run、推理系统或数据 pipeline 是否正确。
\subsection{Decoder-only Transformer 的骨架 的小结}
\begin{itemize}
\item Decoder-only Transformer 的骨架 的任何性能结论都必须同时报告问题规模和评估口径，否则不可比较。
...
现在进入 Normalization、activation 与 position encoding。本节关心的不是名称本身，而是它把 现代 Transformer architecture 与 hyperparameters 中的哪些对象变成可计算、可测量或可优化的量。
...
现代 LLM 常用 RMSNorm 或类似变体以降低开销并改善稳定性。 这给出本节的对象和边界。读者需要把其中的名词对应到张量、数据记录、训练状态或系统状态，而不是停留在缩写层面。
RoPE 把位置信息编码进 query/key 的旋转关系，支持相对位置信息并常见于长上下文模型。 因而同一个方法在小规模示例、单机实验和集群训练中的意义并不相同。比较方法时，问题规模、硬件条件、数据分布和评估口径必须同时给出。
SwiGLU/GeGLU 等 gated MLP 变体体现了课程第一讲的 caveat：有些设计先来自实验，再由经验固化。 这使本节具有可检验性：如果一个结论不能在公式、伪代码、profile trace、benchmark 或 ablation 中表现出来，它就还只是直觉。
因此，Normalization、activation 与 position encoding 应当被理解为一组可以实现和测试的假设。CS336 反复强调 from scratch，正是因为只有能被复现的机制，才可能被稳定改进。
...
检验 Normalization、activation 与 position encoding 的第一步是确定测量对象。这里的测量对象通常不是一个抽象名词，而是与 RMSNorm、RoPE、SwiGLU、pre-norm、position 有关的张量、样本、请求、奖励、通信操作或评估记录。如果对象没有被具体化，后面的实验就只能得到描述性观察，不能得到可复现结论。
第二步是构造最小输入。最小输入应该足够小，使读者能手算或打印关键中间量；同时又要保留本节机制。对于 Normalization、activation 与 position encoding，最小输入不追求逼真规模，而追求暴露因果关系：改变一个变量时，哪些输出、成本或错误会随之改变。
第三步是写出资源账本。账本可以是 FLOPs、bytes moved、显存峰值、通信量、token 数、样本数、reward 分布或 benchmark 分数。符号说明：\textbackslash{}(x\textbackslash{}) 是 hidden vector，\textbackslash{}(g\textbackslash{}) 是可学习缩放，\textbackslash{}(d\textbackslash{}) 是 hidden dimension，\textbackslash{}(\textbackslash{}epsilon\textbackslash{}) 防止除零。 这类说明应被转化为单位明确的数量关系；如果数量只能间接观测，就必须说明使用了什么 proxy。
第四步是设置对照。一个好对照只改变一个因素，例如 batch size、sequence length、vocabulary size、attention heads、filter threshold、learning rate、decoding 参数或 verifier。如果多个因素同时改变，实验最多说明系统表现不同，不能说明是哪一个机制在起作用。
第五步是观察失败条件。教材中的成功路径往往最容易记住，但工程中真正决定方法边界的是失败案例。本节至少要记住这些限制：归一化改善优化，但也可能改变表示尺度和 residual stream 的解释方式；长上下文 extrapolation 不能只靠位置编码，还受训练数据、attention implementation 和评估分布影响。复习时可以把这些限制改写成反例测试。
最后，把实验结论放回 现代 Transformer architecture 与 hyperparameters 的整体结构中。Normalization、activation 与 position encoding 不是孤立技巧：它会影响训练目标、数据选择、系统成本或评估解释中的至少一项。能说清楚这种影响，才说明读者已经从名词记忆进入机制理解。
\subsection{常见混淆：Normalization、activation 与 position encoding}
normalization（归一化）影响优化稳定性和 residual path。在“Normalization、activation 与 position encoding”中，这个概念用于解释 现代 Transformer architecture 与 hyperparameters 的一个具体机制。
RoPE（rotary positional embedding）把位置信息编码进 query/key 的旋转关系。在“Normalization、activation 与 position encoding”中，这个概念用于解释 现代 Transformer architecture 与 hyperparameters 的一个具体机制。
SwiGLU 在本节中应被看作一个技术对象，而不是一个孤立名词。需要判断它指向模型、数据、系统还是评估，并说明它在“Normalization、activation 与 position encoding”中改变了哪个变量或约束。
normalization（归一化）影响优化稳定性和 residual path。在“Normalization、activation 与 position encoding”中，这个概念用于解释 现代 Transformer architecture 与 hyperparameters 的一个具体机制。
position 在本节中应被看作一个技术对象，而不是一个孤立名词。需要判断它指向模型、数据、系统还是评估，并说明它在“Normalization、activation 与 position encoding”中改变了哪个变量或约束。
这些概念的共同点是：只有进入公式、代码或评估协议之后才有明确含义。单独背诵名称不会帮助判断一个训练 run、推理系统或数据 pipeline 是否正确。
\subsection{Normalization、activation 与 position encoding 的小结}
\begin{itemize}
\item Normalization、activation 与 position encoding 的任何性能结论都必须同时报告问题规模和评估口径，否则不可比较。
\item 如果一个方法声称降低主要成本，它通常会把成本转移到另一个变量，例如显存、通信、数据质量、训练稳定性或评估复杂度。
\item 公式和伪代码提供推理骨架；真实系统仍需要 profiler、ablation 和 held-out evaluation。
\end{itemize}
\section{Hyperparameter 的规模化视角}
现在进入 Hyperparameter 的规模化视角。本节关心的不是名称本身，而是它把 现代 Transformer architecture 与 hyperparameters 中的哪些对象变成可计算、可测量或可优化的量。
...
lecture 3 的核心态度是：不要把 hyperparameters 当成小模型实验后的装饰参数，它们决定大规模训练的稳定性和资源效率。 这给出本节的对象和边界。读者需要把其中的名词对应到张量、数据记录、训练状态或系统状态，而不是停留在缩写层面。
batch size、learning rate、warmup、decay、weight decay、dropout、context length 和 width/depth ratio 都进入 compute-budget 约束。 因而同一个方法在小规模示例、单机实验和集群训练中的意义并不相同。比较方法时，问题规模、硬件条件、数据分布和评估口径必须同时给出。
本书需要把每个超参数和其 failure mode 绑定：divergence、undertraining、overfitting、memory blow-up、communication bottleneck。 这使本节具有可检验性：如果一个结论不能在公式、伪代码、profile trace、benchmark 或 ablation 中表现出来，它就还只是直觉。
因此，Hyperparameter 的规模化视角 应当被理解为一组可以实现和测试的假设。CS336 反复强调 from scratch，正是因为只有能被复现的机制，才可能被稳定改进。
...
检验 Hyperparameter 的规模化视角 的第一步是确定测量对象。这里的测量对象通常不是一个抽象名词，而是与 hyperparameter、learning rate、AdamW、warmup、scale 有关的张量、样本、请求、奖励、通信操作或评估记录。如果对象没有被具体化，后面的实验就只能得到描述性观察，不能得到可复现结论。
第二步是构造最小输入。最小输入应该足够小，使读者能手算或打印关键中间量；同时又要保留本节机制。对于 Hyperparameter 的规模化视角，最小输入不追求逼真规模，而追求暴露因果关系：改变一个变量时，哪些输出、成本或错误会随之改变。
第三步是写出资源账本。账本可以是 FLOPs、bytes moved、显存峰值、通信量、token 数、样本数、reward 分布或 benchmark 分数。符号说明：这是 AdamW 风格更新；\textbackslash{}(\textbackslash{}eta\_t\textbackslash{}) 是学习率，\textbackslash{}(\textbackslash{}hat m\_t,\textbackslash{}hat v\_t\textbackslash{}) 是矩估计，\textbackslash{}(\textbackslash{}lambda\textbackslash{}) 是 decoupled weight decay。 这类说明应被转化为单位明确的数量关系；如果数量只能间接观测，就必须说明使用了什么 proxy。
第四步是设置对照。一个好对照只改变一个因素，例如 batch size、sequence length、vocabulary size、attention heads、filter threshold、learning rate、decoding 参数或 verifier。如果多个因素同时改变，实验最多说明系统表现不同，不能说明是哪一个机制在起作用。
第五步是观察失败条件。教材中的成功路径往往最容易记住，但工程中真正决定方法边界的是失败案例。本节至少要记住这些限制：小规模最优超参数不一定可直接外推；hyperparameter search 的成本本身也是 scaling plan 的一部分。复习时可以把这些限制改写成反例测试。
最后，把实验结论放回 现代 Transformer architecture 与 hyperparameters 的整体结构中。Hyperparameter 的规模化视角 不是孤立技巧：它会影响训练目标、数据选择、系统成本或评估解释中的至少一项。能说清楚这种影响，才说明读者已经从名词记忆进入机制理解。
\subsection{常见混淆：Hyperparameter 的规模化视角}
hyperparameter 在本节中应被看作一个技术对象，而不是一个孤立名词。需要判断它指向模型、数据、系统还是评估，并说明它在“Hyperparameter 的规模化视角”中改变了哪个变量或约束。
learning rate 在本节中应被看作一个技术对象，而不是一个孤立名词。需要判断它指向模型、数据、系统还是评估，并说明它在“Hyperparameter 的规模化视角”中改变了哪个变量或约束。
AdamW 在本节中应被看作一个技术对象，而不是一个孤立名词。需要判断它指向模型、数据、系统还是评估，并说明它在“Hyperparameter 的规模化视角”中改变了哪个变量或约束。
warmup 在本节中应被看作一个技术对象，而不是一个孤立名词。需要判断它指向模型、数据、系统还是评估，并说明它在“Hyperparameter 的规模化视角”中改变了哪个变量或约束。
scale 在本节中应被看作一个技术对象，而不是一个孤立名词。需要判断它指向模型、数据、系统还是评估，并说明它在“Hyperparameter 的规模化视角”中改变了哪个变量或约束。
这些概念的共同点是：只有进入公式、代码或评估协议之后才有明确含义。单独背诵名称不会帮助判断一个训练 run、推理系统或数据 pipeline 是否正确。
\subsection{Hyperparameter 的规模化视角 的小结}
\begin{itemize}
\item Hyperparameter 的规模化视角 的任何性能结论都必须同时报告问题规模和评估口径，否则不可比较。
\item 如果一个方法声称降低主要成本，它通常会把成本转移到另一个变量，例如显存、通信、数据质量、训练稳定性或评估复杂度。
\item 公式和伪代码提供推理骨架；真实系统仍需要 profiler、ablation 和 held-out evaluation。
\end{itemize}
