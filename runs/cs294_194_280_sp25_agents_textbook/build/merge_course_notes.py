...
def title_from_manifest(manifest: dict) -> str:
    return manifest.get("course_title", "CS294/194-280 全课程讲义")


def main() -> None:
    manifest = json.loads((BUILD_DIR / "course_manifest.json").read_text())
    tex_path = BUILD_DIR / "cs294_194_280_complete_notes.tex"
    title = latex_escape(title_from_manifest(manifest))
